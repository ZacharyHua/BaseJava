...
public class Demo7 {
    ...
    /**
     * 1、线程池信息：核心线程数量5，最大线程数量10，无界队列，超出核心线程数量的线程存活时间：5秒， 指定拒绝策略
     *
     * @throws Exception
     */
    private void threadPoolExecutorTest1() throws Exception{
        ...
        // 预计结果：线程池线程数量为：5，超出数量的任务，其他进入等待队列中等待执行
    }

    /**
     * 2、 线程池信息： 核心线程数量5，最大线程数量10，队列大小3，超出核心线程数量的线程存货时间：5秒，指定拒绝策略
     *
     * @throws Exception
     */
    private void threadPoolExecutorTest2() throws Exception{
        ...
        // 4、队列和线程池都满了，剩下两个任务，没有资源了，被拒绝执行
        // 5、任务执行，5秒后，如果无任务可执行，销毁临时创建的5个线程
    }

    /**
     * 3、 线程池信息： 信息线程数量5，最大数量5，无界队列，超出核心线程数量的线程存活时间：5秒
     *
     * @throws Exception
     */
    private void threadPoolExecutorTest3() throws Exception{
        ...
        testCommon(threadPoolExecutor);
        //预计结果：线程池线程数量：5，超出数量任务，其他的进入队列中等待被执行
    }
    ...
    private void threadPoolExecutorTest4() throws Exception{
        // SynchronousQueue，实际上它不是一个真正的队列，因为它不会为队列中元素的维护存储空间。与其他队列不同的是，它玩维护一组线程，这些线程在等待着把元素加入或者移除队列
        // 在上会用SynchronousQueue作为工作队列的前提，客户端代码向线程池提交任务时。
        // 而线程池又没有空闲的线程能够从SynchronousQueue队列实例中取一个任务
        ...
        testCommon(threadPoolExecutor);
        // 预计结果：
        // 1、线程池线程数量为：15，超出线程数量的任务，其他的进入队列中等待被执行
     ...
     *      核心线程池数量5，最大线程池数量Integer.MAX_VALUE，DelayedWorkQueue延时队列，超出线程数量的线程存活时间：0秒
     *
     * @throws Exception
     */
    private void threadPoolExecutorTest5() throws Exception {
        ...
        threadPoolExecutor.schedule(new Runnable() {
            @Override
            public void run() {
                System.out.println("任务正在被执行，现在时间："+System.currentTimeMillis());
            }
        },3000,TimeUnit.MILLISECONDS);
        System.out.println("定时任务，提交成功，时间是："+System.currentTimeMillis()+" , 当前线程池的数量："+threadPoolExecutor.getPoolSize());
        // 预计结果：任务在3秒后被执行一次
    }

    /**
     * 6、 定时执行线程池信息，线程固定数量为5
